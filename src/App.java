...
public class App {
    Connection con = DriverManager.getConnection("jdbc:postgresql://localhost:5432/cats",
            "postgres",
            "postgres");
    ...
    public void inputHandler() throws SQLException {
        switch (args[0]) {
            ...
            case "2":
                ...
                break;
            case "3": {
                ResultSet rs = stmt.executeQuery("SELECT *, DATE_PART('YEAR', Age(birthsday)) FROM employees " +
                        "ORDER by name ASC;");
                while (rs.next()) {
            ...
            case "5": {
                ...
                while (rs.next()) {
                    System.out.println(rs.getString(1) + "\t\t" + rs.getString(2)
                            + "\t" + rs.getString(3)  + "\t" + Record.getAge(rs.getString(2)));
                }
